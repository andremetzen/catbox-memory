...
exports = module.exports = internals.Connection = class {
    ...
    start() {
        ...
        if (!this.cache) {
            this.cache = new LFU(this.settings.maxByteSize);
        }
    }
    ...
    get(key) {
        ...
        const envelope = this.cache.get(this.getStringKey(key));
    ...
    set(key, value, ttl) {
        ...
        const cachedItem = this.cache.get(this.getStringKey(key));
        if (cachedItem &&
        ...
        this.cache.put(this.getStringKey(key), envelope);
    }
